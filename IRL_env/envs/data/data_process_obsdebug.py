...
class natural_road_data():
    def __init__(self, lane_id):   
        ...
        self.obstacle_trajectory03 = []  # [x, y, V] km/h
    ...
    def read_from_csv(self, filepath):
        ...
        obs03_filename = os.path.join(filepath, 'obs_trajectory03.csv')
        with open(obs03_filename, mode='r') as csv_file:
            csv_reader = csv.DictReader(csv_file)
            for row in csv_reader:
                if self.ego_lane_id == 0:
                    obstacle_x03 = float(row['ad_right_x'])
                    obstacle_y03 = float(row['ad_right_y'])
                else:
                    obstacle_x03 = float(row['ad_left_x'])
                    obstacle_y03 = float(row['ad_left_y'])
                obstacle_speed03 = float(42)
                self.obstacle_trajectory03.append([obstacle_x03, obstacle_y03, obstacle_speed03, 1 - self.ego_lane_id])
        ...
        self.obstacle_trajectory = np.array(self.obstacle_trajectory)[3:166,:]
        self.obstacle_trajectory03 = np.array(self.obstacle_trajectory03)[3:166,:]
    ...
    def build_trajectory(self): # id=0 outside lane, id=1 inside lane  
        # filepath = '/home/jx/Yangjiaxin/IRL/IRL_env/envs/data/processed/'
        filepath = 'G:\Jiaxin_OneDrive\OneDrive\桌面\carla_road\IRL_poly_newRP\IRL_env\envs\data\processed\\'
        self.read_from_csv(filepath)
        return self.expert_trajectory, self.road, self.obstacle_trajectory, self.obstacle_trajectory03

if __name__ == '__main__':
    env_data = natural_road_data(lane_id=1)
    expert_trajectory, road, obstacle_trajectory, obstacle_trajectory03 = env_data.build_trajectory()
    ...
    plt.plot(obstacle_trajectory03[:,0], obstacle_trajectory03[:,1], 'r--', label='obstacle trajectory 0.3')
    plt.plot(road[:, 0], road[:, 1], color='k')
    plt.plot(road[:, 2], road[:, 3], color='k')
    plt.plot(road[:, 4], road[:, 5], 'k--', label='road center')
    plt.axis('equal')
