...
#----#
MAX_ROAD_WIDTH = 0.8  # maximum road width [m]  int(4/2-1.85/2) 车道减去车宽
...
#末状态只采样了横向位置和纵向速度，其他末状态认为0
#初状态d_dot认为0不合理，会导致生成的曲线不平滑，因为我们会一个点一个点计算，初状态不可能为0.不过这里初状态d_dot_dot认为0
...
def calc_frenet_path(lat_param, lon_param):
    """
        横向五次多项式，纵向四次多项式。根据终点横向位置，终点纵向速度计算出一条frenet轨迹
    :return: 一条frenet轨迹
    """
    fp = FrenetPath()
...
def calc_frenet_paths(csp, c_speed, c_accel, c_d, c_d_d, c_d_dd, s0, planner_param, TARGET_SPEED):
    ...
    MAX_SPEED_2 = (TARGET_SPEED/3.6-40/3.6)**2  # maximum speed error[m/s]
    MAX_FRENET_2 = (TARGET_SPEED/3.6*5.-40/3.6*5.0)**2 # maximum frenet error[m]
    ...
    # Lateral motion planning
        ...
        for tv in np.arange(TARGET_SPEED - D_T_S * N_S_SAMPLE,
            ...
            ds = 2 * ds / MAX_SPEED_2 -1
            df = 2 * df / MAX_FRENET_2 -1

            cdd = 2 * (tfp.d[-1] ** 2) / (MAX_ROAD_WIDTH ** 2) - 1
            tfp.cd = planner_param[0] * Jp + planner_param[1] * cdd + (1.0 - planner_param[0] - planner_param[1]) * (tfp.d[-1])
...
def calc_global_paths(fp, csp):
    ...
    for i in range(len(fp.s)):
        if fp.s[i] <= 0:
            ...
            i_kr = csp.calc_curvature(0)
        else:
            ix, iy = csp.calc_position(fp.s[i])
            i_yaw = csp.calc_yaw(fp.s[i])
            i_kr = csp.calc_curvature(fp.s[i])
        i_kr_list.append(i_kr)
        i_yaw_list.append(i_yaw)
        if ix is None:
            break
    ...
    # calc yaw and ds
    if len(fp.s_d) > 0:
        d_s = fp.d_d/np.where(np.abs(fp.s_d)<1e-6, np.sign(fp.s_d)*1e-6, fp.s_d)
    else:
        d_s = np.gradient(fp.d, fp.s)
    denominator = 1 - fp.d * np.array(i_kr_list)
    denominator = np.where(np.abs(denominator)<1e-6, np.sign(denominator)*1e-6, denominator)
    fp.yaw = i_yaw_list + np.arctan2(d_s, denominator)
...
class PolyPlanner():
    ...
    def calculate_frenet_coordinates(self, x, y, speed, yaw, ax=0, ay=0, mode=0):
        """
            计算 (x, y) 在参考线上的 frenet 坐标
        Param
            mode: 0: 不考虑ax,ay，认为s_dot_dot,d_dot_dot=0 1: 正常计算   
        ...
        s_dot = np.dot(vector_speed,vector_ref)/(1-d*self.tc[nearest_index])
        ...
        if mode == 0:
            ...
            vector_ref_n = [-np.sin(self.tyaw[nearest_index]),np.cos(self.tyaw[nearest_index])]
            a_h = [ax,ay]
            curva_r = self.tc[nearest_index]
            # 近似认为曲率对s导数为0
            d_dot_dot = np.dot(a_h,vector_ref_n)- curva_r*(1-curva_r*d)*s_dot**2
            s_dot_dot = (1.0/(1.0-curva_r*d))*(np.dot(a_h,vector_ref)+2*curva_r*d_s*s_dot**2)

        return s, s_dot, s_dot_dot, d, d_dot, d_dot_dot

    def poly_trajectory(self, ego_x, ego_y, ego_speed, ego_yaw,planner_param, target_speed, ob=np.array([])):
        s0, c_speed, c_accel, c_d, c_d_d, c_d_dd = self.calculate_frenet_coordinates(ego_x, ego_y, ego_speed, ego_yaw)
    ...
    def debug_sim_frenet_planT(self):
        ...
        param_list.append([0.0, 0.8])   # mean velocity= 12.4 m/s, end offset= -0.25 m
        param_list.append([0.0, 0.9])   # mean velocity= 12.4 m/s, end offset= -0.25 m
        ...
        # # # param_list.append([0.6, 0.5])   # mean velocity= 12.7 m/s, end offset= 0.0 m [K_T=0.0]
        # param_list.append([0.0, 0.0])   # mean velocity= 12.7 m/s, end offset= 0.0 m [K_T=0.0]
        ...
        for i in range(SIM_LOOP):
            ...
            # path = self.poly_trajectory(ego_x, ego_y, ego_speed, ob)
            path = self.poly_trajectory(ego_x, ego_y, ego_speed, ego_yaw, planner_param, target_speed, ob)
            end_offset = path.d[-1]
            x = path.s
            y = path.d
            velocity = path.s_d
...
if __name__ == '__main__':
    env_data = natural_road_data(lane_id=0)
    planner = PolyPlanner(env_data, lane_id=0)
