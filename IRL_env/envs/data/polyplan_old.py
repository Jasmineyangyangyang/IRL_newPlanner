...
from trajectory_planner.CubicSpline import cubic_spline_planner
from data.data_process import natural_road_data

# for debug
# from data_process import natural_road_data
...
MAX_SPEED_2 = 2  # maximum speed error[m/s]
MAX_FRENET = 70 # maximum frenet error[m]
#--------#
MAX_ROAD_WIDTH = 1.0  # maximum road width [m]  int(4/2-1.85/2) 车道减去车宽
D_ROAD_W = 0.25  # road width sampling length [m]
DT = 0.25  # firts searching time tick [s]
DT_best_path = 0.1  # best path searching time tick [s]
PLAN_T = 5.0  # max prediction time [m]
TARGET_SPEED = 42.0 / 3.6  # target speed [m/s]
...
def calc_frenet_paths(c_speed, c_accel, c_d, c_d_d, c_d_dd, s0, planner_param):
    fp = FrenetPath()
    target_v = planner_param[0]  # target speed
    target_d = planner_param[1]  # target lateral offset
    Ti = PLAN_T

    # lateral motion planning (quintic polynomial)
    lat_qp = QuinticPolynomial(c_d, c_d_d, c_d_dd, target_d,  Ti)
    fp.t = [t for t in np.arange(0.0, Ti+DT_best_path, DT_best_path)]
    fp.d = [lat_qp.calc_point(t) for t in fp.t]
    fp.d_d = [lat_qp.calc_first_derivative(t) for t in fp.t]
    fp.d_dd = [lat_qp.calc_second_derivative(t) for t in fp.t]
    fp.d_ddd = [lat_qp.calc_third_derivative(t) for t in fp.t]
    fp.lat_param = [c_d, c_d_d, c_d_dd, target_d, Ti]
    # Longitudinal motion planning (Velocity keeping)
    lon_qp = QuarticPolynomial(s0, c_speed, c_accel, target_v, Ti)
    fp.s = [lon_qp.calc_point(t) for t in fp.t]
    fp.s_d = [lon_qp.calc_first_derivative(t) for t in fp.t]
    fp.s_dd = [lon_qp.calc_second_derivative(t) for t in fp.t]
    fp.s_ddd = [lon_qp.calc_third_derivative(t) for t in fp.t]
    fp.lon_param = [s0, c_speed, c_accel, target_v, Ti]
...
def frenet_optimal_planning(csp, s0, c_speed, c_accel, c_d, c_d_d, c_d_dd, planner_param, ob):  
    fplist_1 = calc_frenet_paths(c_speed, c_accel, c_d, c_d_d, c_d_dd, s0, planner_param)

    fp = calc_global_paths(fplist_1, csp)
...
class PolyPlanner():
    ...
    def find_nearest_point(self, x, y):
        # Calculate the distance from each reference point to (x, y)
        distances = np.sqrt((np.array(self.tx) - x)**2 + (np.array(self.ty) - y)**2)
        # Find the index of the nearest reference point
        nearest_index = np.argmin(distances)
        
        return nearest_index
    def calculate_frenet_coordinates(self, x, y, speed):
        ...
        # Get the accumulated distance s of the nearest point
        s = 0.1 *nearest_index
        
        # Calculate the perpendicular distance d from the projection point to (x, y)
        dx = x - cx
        dy = y - cy
    ...
    def poly_trajectory(self, ego_x, ego_y, ego_speed, planner_param, ob=np.array([])):
        s0, c_speed, c_accel, c_d, c_d_d, c_d_dd = self.calculate_frenet_coordinates(ego_x, ego_y, ego_speed)

        path = frenet_optimal_planning(self.csp, s0, c_speed, c_accel, c_d, c_d_d, c_d_dd, planner_param, ob)
        return path
    ...
    def debug_sim_global_all(self):
        ...
        for param in param_list:
            ...
            for i in range(SIM_LOOP):
                ...
                path = self.poly_trajectory(ego_x, ego_y, ego_speed, planner_param, ob)  # add ego 0.1s后的state
                # end_time = time.time()
    ...
    def debug_sim_frenet_all(self):
        ...
        ego_y = self.ty[0]
        ego_speed = 42.0 / 3.6  # current speed [m/s]
        target_v = 50.0 / 3.6  # target speed [m/s]
        target_d = 0.0  # target lateral offset [m]
        planner_param = [target_v, target_d] 
        ob = np.array([])
        ...
        plt.plot(np.linspace(0, 430, 100), np.ones(100)*1.0, 'k--', linewidth=1)
        plt.plot(np.linspace(0, 430, 100), np.ones(100)*-1.0, 'k--', linewidth=1)
        plt.xlabel("x[m]")
        plt.ylabel("y[m]")
        ...
        for param in param_list:
            ...
            for i in range(SIM_LOOP):
                ...
                pathlist = self.poly_trajectory(ego_x, ego_y, ego_speed, planner_param, ob)
    ...
    def debug_sim_frenet_planT(self):
        ...
        for vi in np.linspace(40., 55.1, 8):
            for di in np.linspace(-1.01, 1.01, 11):
                planner_param = [vi/3.6, di]
                param_list.append(planner_param)
        ...
        plt.plot(np.linspace(0, 191, 100), np.ones(100)*1.0, '--', color='lime',linewidth=1)
        plt.plot(np.linspace(0, 191, 100), np.ones(100)*-1.0, '--', color='lime', linewidth=1)

        for i in range(SIM_LOOP):
            planner_param = param_list[i]
            # path = self.poly_trajectory(ego_x, ego_y, ego_speed, ob)
            path = self.poly_trajectory(ego_x, ego_y, ego_speed, planner_param, ob)
            end_offset = path.d[-1]
            x = path.s
            y = path.d
            velocity = path.s_d
