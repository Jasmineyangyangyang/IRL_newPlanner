...
class natural_road_load():
    ...
    def read_from_csv(self, filepath='/Users/jiaxin/codex/IRL_newCostPlanner/IRL_env/envs/data/processed'):
    ...
    def load_trajectory_csv(self, filepath):
        ...
        with open(ego_filename, mode='r') as csv_file:
            ...
            for row in csv_reader:
                if self.ego_lane_id == 0:
                    ...
                    expert_yaw = float(row['left_center_yaw'])
                    expert_speed = float(row['left_spdms'])
                    expert_curvature = float(row['leftcenter_curv'])
                else:
                    expert_x = float(row['right_center_x'])
                    expert_y = float(row['right_center_y'])
                    expert_yaw = float(row['right_center_yaw'])
                    expert_speed = float(row['right_spdms'])
                    expert_curvature = float(row['rightcenter_curv'])

                expert_trajectory.append([expert_x, expert_y, expert_yaw, expert_speed, expert_curvature, self.ego_lane_id])
        ...
        with open(obs_filename, mode='r') as csv_file:
            ...
            for row in csv_reader:
                if self.ego_lane_id == 0:
                    ...
                    obstacle_yaw = float(row['ad_right_yaw'])
                    obstacle_speed = float(row['ad_right_spdms'])
                    obstacle_curvature = float(row['ad_right_curv'])
                else:
                    obstacle_x = float(row['ad_left_x'])
                    obstacle_y = float(row['ad_left_y'])
                    obstacle_yaw = float(row['ad_left_yaw'])
                    obstacle_speed = float(row['ad_left_spdms'])
                    obstacle_curvature = float(row['ad_left_curv'])

                obstacle_trajectory.append([obstacle_x, obstacle_y, obstacle_yaw, obstacle_speed, obstacle_curvature, 1 - self.ego_lane_id])
        ...
        obstacle_trajectory = np.array(obstacle_trajectory)[11:174,:] 
        road = np.array(road)[:600,:]    #lhl 计算时候TLC需要更长的道路信息
        print("Load data successfully!")
        return expert_trajectory, road, obstacle_trajectory

    def build_trajectory(self, filepath='./'):
        filepath = '/Users/jiaxin/codex/IRL_newCostPlanner/IRL_env/envs/data/processed'
        expert_trajectory, road_trajectory, obstacle_trajectory = self.load_trajectory_csv(filepath)
        return expert_trajectory, road_trajectory, obstacle_trajectory
...
if __name__ == '__main__':
    ...
    ego_lane_id = 1
    env_data = natural_road_load(ego_lane_id)
    road = env_data.read_from_csv()
    ego_trajectory, road, obstacle_trajectory = env_data.build_trajectory()
    ...
    plt.figure(260313)
    plt.plot(road_left[:,0], road_left[:,1], 'k--', label='road')
    ...
    if ego_lane_id == 0:
        nearest_index = np.argmin(np.sqrt((road_center[:,0] - 402.02)**2 + (road_center[:,1] - -237.79)**2))
        ...
        nearest_index = np.argmin(np.sqrt((road_center[:,0] - 402.214)**2 + (road_center[:,1] - -245.289)**2))
